...
|--------|------------|------------------|------|
| ① 当日计数口径三处行号 | `:204-208 / :247-252 / :271-276`（M2 落码后漂移） | **终审裁定（@Hermes 2026-09-16，HEAD `bcd202c` 工作树直读）= `:205` / `:248` / `:273`**（SQL 字符串字面量行口径；归属按 HEAD `2a7a5bd` 实测更正：`:205` = `check_quota` 兜底路径（`check_quota_sync` 口径）/ `:248` = `check_credits` / `:273` = `check_quota` 主路径，`get_quota_status_sync` 本身无独立计数行不属「三处」之列，此前误归已勘误；v0.3 一度按 @Codex 核认报告误引参数行 `:206/:249/:274`，已回滚作废仅作版本对照保留）；`since = _today_start_utc()` 定义 = `:17` 不变 | B 草案 / M3 风险清单引用以 `:205/:248/:273`（SQL 字面量行终审基准，M4 落码 PR 合入后再随 commit 核一次）为准；口径统一 = `COUNT(*) WHERE user_id=? AND generated_at >= _today_start_utc()`，与「整篇计一次」同源 |
