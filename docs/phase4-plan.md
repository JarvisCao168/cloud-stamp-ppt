...
|--------|------------|------------------|------|
| ① 当日计数口径三处行号 | `:204-208 / :247-252 / :271-276`（M2 落码后漂移） | 台账漂移值（v0.2.4 首落）`COUNT(*) WHERE user_id=? AND generated_at >= ?` 三处 = `:205` / `:248` / `:273`，**v0.3 按 @Codex M3 实码核认报告（HEAD `bcd202c`）复核实测 = `:206` / `:249` / `:274`**（`get_quota_status_sync` / `check_credits` / `check_quota` 各一处，口径本身三处统一无误）；`since = _today_start_utc()` 定义 = `:17` 不变 | B 草案 / M3 风险清单引用以 `:206/:249/:274`（@Codex 核认报告，M4 落码 PR 合入后再随 commit 核一次）为准；口径统一 = `COUNT(*) WHERE user_id=? AND generated_at >= _today_start_utc()`，与「整篇计一次」同源 |
...
| v0.2.5 | 2026-09-16 | M3 实码核认报告（@Codex）复核后行号勘误同步（Claude）：§4·B.6 勘误行 ① 当日计数口径三处 COUNT 行号由 v0.2.4 台账漂移值 `:205/:248/:273` 修正为 HEAD `bcd202c` 实测 `:206/:249/:274`（口径本身三处统一不变）；头部状态行同步更新；`docs/m3-b-sse-incremental-events-draft.md` 升 v0.3（§九 修正登记 3 处：§四.4 行号勘误 + §四.1 调用点/定义措辞归位 + §四.2 单测锚点补 `:99-100` `full_control` 同路径验证点） |
