...
**日期**: 2026-09-14
**作者**: Claude（首席架构师）
**状态**: 设计稿 v0.2.4（M3 启动令 A+B 裁定（@JARVIS 2026-09-16）；v0.2.5-generation 补录（@Claude，@Hermes v0.7 方案裁定照单执行，2026-09-16）：§4·B.3 S1-S2 / §4·B.4 预扣点 / §4·B.5 硬约束 ③ / §4·B.6 新增勘误行 ④ generation.py 行号基准按 HEAD `2a7a5bd` 工作树直读补录（M4 执行基准：`:751/:756/:757/:759/:761-772/:776-782/:792-806/:822` + 总线侧 `:45/:56-64/:67-75/:79-131/:132-191/:136-138`），§4·B.6 勘误行 ① 归属标注维持终审口径 `:205`=get_quota_status_sync / `:248`=check_credits / `:273`=check_quota（v0.6 归属更正部分经实码复核作废丢弃），§4.1-4.4 既有锁定内容零改动；零代码变更）：新增 §4·B M3 B 案 SSE 增量事件（presence）协议草案登记——独立 `event` 命名空间（`viewer_joined`/`viewer_left`/`presence_snapshot`）+ payload「只增不删不改义」+ 分发点清单 D1-D5/S1-S2 + 预扣点章节引用组长终审裁定 #4 原文「整篇计一次」+ 429/402 路径分离标注 + 基座 `098cde9` 不动声明 + 三条收口硬约束落档（含 `config.py` 完整路径勘误）+ 实码勘误行（当日计数口径三处行号终审裁定 = `:205/:248/:273`（SQL 字符串字面量行口径，@Hermes M3 终审 2026-09-16；v0.3 一度按 @Codex 核认报告误引参数行 `:206/:249/:274`，已回滚作废仅作版本对照保留），以 §4·B.6 勘误行 ① 终审裁定为准）；§4.1-4.4 既有锁定内容零改动；M3 零生产代码变更，B 落码在 M4。v0.2.3 §3.5 M2 预扣点接入设计 + v0.2.2.4 行号实测锁定基线不变。JARVIS 终审基准 = 本稿落档 commit）
...
|--------|------------|------------------|------|
| ① 当日计数口径三处行号 | `:204-208 / :247-252 / :271-276`（M2 落码后漂移） | **终审裁定（@Hermes 2026-09-16，HEAD `bcd202c` 工作树直读）= `:205` / `:248` / `:273`**（SQL 字符串字面量行口径；`get_quota_status_sync` / `check_credits` / `check_quota` 各一处，口径本身三处统一无误；v0.3 一度按 @Codex 核认报告误引参数行 `:206/:249/:274`，已回滚作废仅作版本对照保留）；`since = _today_start_utc()` 定义 = `:17` 不变 | B 草案 / M3 风险清单引用以 `:205/:248/:273`（SQL 字面量行终审基准，M4 落码 PR 合入后再随 commit 核一次）为准；口径统一 = `COUNT(*) WHERE user_id=? AND generated_at >= _today_start_utc()`，与「整篇计一次」同源 |
...
| v0.2.5 | 2026-09-16 | M3 实码核认报告（@Codex）复核后行号勘误同步（Claude）：§4·B.6 勘误行 ① 当日计数口径三处 COUNT 行号由 v0.2.4 台账漂移值 `:205/:248/:273` 一度修正为 @Codex 报告所引 `:206/:249/:274`；**@Hermes M3 终审裁定（2026-09-16）：`:206/:249/:274` 系 COUNT 语句参数行（`user_id, since`）误引，非 SQL 字符串字面量行——`HEAD bcd202c` 工作树直读终审基准 = `:205`（`get_quota_status_sync`）/ `:248`（`check_credits`）/ `:273`（`check_quota`），v0.2.4 首落值正确，v0.2.5 行号落改回滚作废仅作版本对照保留，「台账漂移 +1 行」结论一并作废；§4·B.6 勘误行 ① 已同步回滚**；头部状态行同步更新；`docs/m3-b-sse-incremental-events-draft.md` 升 v0.3（§九 修正登记 3 处：§四.4 行号勘误 + §四.1 调用点/定义措辞归位 + §四.2 单测锚点补 `:99-100` `full_control` 同路径验证点；§四.4 行号按 @Hermes 终审裁定回滚为 `:205/:248/:273`） |
...
| v0.2.5-generation | 2026-09-16 | M4 generation.py 行号基准补录（@Claude，@Hermes v0.7 方案裁定照单执行；取 v0.6 行号补录部分 + v0.5 归属部分，丢弃 v0.6 归属更正部分，零代码变更）：§4·B.3 S1-S2 后端行号补录（`collab_publish()` `:56-64` + `_COLLAB_MAX_LOG` `:45`，回放路径 `:136-138`；`collab_stream()` 端点 `:79-131` + `event_stream()` `:132-191` + `_collab_snapshot()` `:67-75`）；§4·B.4 预扣点调用点 `:685→:756`（M4 执行基准，M3 核认时点保留对照）+ `record_usage` 调用点 `:822`；§4·B.5 硬约束 ③ NIT 判定式行号补录（判定式注释 `:759` + 三分支 `:761-772`/`:776-782`/`:792-806`）；§4·B.6 新增勘误行 ④（generation.py 行号基准补录全表，M2 落码后下游漂移修正，quota.py 行号维持终审基准不变）；§4·B.6 勘误行 ① 补 v0.6 作废归注；头部状态行升 v0.2.5-generation 登记 |
