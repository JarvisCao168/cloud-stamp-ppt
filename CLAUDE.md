...
- **Quota / credits** (`backend/app/core/quota.py`): daily free-quota counter (`usage_log`, `FREE_DAILY_LIMIT=10`) + Phase 4 credit system (`user_credits` + `credit_ledger`, optimistic-lock deduction in `debit_credits`, retry 3× with 50ms backoff). `/create` checks quota then credits; 429 = free-quota exhausted with balance 0, 402 = insufficient credits — separate paths. `GET /api/quota/status` resolves `user_id` the same three-tier way as `/create` (`X-User-Id` header → `anon-{IP}` → `anon-unknown`); cross-user_id requests 404, never enumerate. `reserve_credit()` (M5-A `901dfa2`) is wired into `/create` at `generation.py:755` (before the `credit_allowed` check) — collaborative/full_control modes no longer early-return 0 (M5-B `a525605` removed the two `return 0` lines in `estimate_required`); they now price off the same input_len tiers as quick (≤500→1, ≤4000→3, ≤8000→5, >8000→8), multimodal→6 retained. The 402 paths: line :776 pre-check (balance > 0, insufficient) and line :792 debit_fail fallback (debit_credits returns -1/-2).
...
- Backend: pytest; quota regression lives in `backend/tests/test_quota_m2.py` — M5 anchors at lines 100–101: `estimate_required("collaborative", 5000, "auto") == 5` and `estimate_required("full_control", 5000, "auto") == 5` (collaborative/full_control no longer return 0 since `a525605`).
