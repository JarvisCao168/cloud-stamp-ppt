...
- **启动令第 3 步（@Claude）已交付**：
  ...
  - **实码勘误行（M4 落码时随 commit 再核）**：当日计数口径三处行号终审裁定（@Hermes 2026-09-16，HEAD `bcd202c` 工作树直读）= `:205`（`get_quota_status_sync`）/ `:248`（`check_credits`）/ `:273`（`check_quota`），SQL 字符串字面量行口径（`since = _today_start_utc()` 定义 = `quota.py:17` 不变）；v0.3 一度按 @Codex 核认报告误引参数行 `:206/:249/:274`，已回滚作废仅作版本对照保留（「台账漂移 +1 行」结论作废）；M4 落码 PR 合入后再随 commit 复核一次
