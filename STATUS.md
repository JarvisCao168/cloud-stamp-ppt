...
**更新日期**: 2026-09-16
**当前阶段**: M4 正式启动（JARVIS 裁定锁定 + @Hermes 终审回执）：Gamma 单点 4 维度 1 天（@Codex，纯调研 → `docs/gamma-competitive-analysis.md`）→ B1（`quota.py:39-40` 预扣点 `mode="collaborative"` 落码 + `test_quota_m2.py:99-100` 回归 + 429/402 零交叉）→ B2（`useCollabStream.ts` D1-D5，Vitest 全绿 + E2E 55/56，验收基线含③「user_id 缺失/`null` → 渲染「未上报」+ `-1`，禁止默认 1」分支断言）→ B3（`test_presence_b3.py` 覆盖率 ≥ 85%，判定函数归属行引 `generation.py:761/776/792`）→ M4 收口 commit（4 项：#1 429/402 验收基线文档 `docs/m4-429-402-acceptance-baseline.md` 并入 / #2 generation.py 行号偏移登记索引（正文已随 `4582fa6`+`c14a5bd`，零内容变更）/ #3 phase4-plan.md 并入项撤销（工作树实读 clean，`a2134ef` 已收口）/ #4 台账行 + 基座 `098cde9` 零改动声明）。B 草案 v0.4 落改 ①③④⑤ 独立 commit 先行（`8f8d625`，B2 开笔前）；L44/L45 勘误（`quota.py:262`→`:219`、`:214`→`generation.py:778`）随 `d56b8ae` 入链。远端归位 `d56b8ae`（零分叉），@Claude 协议侧 429/402 验收基线 B1 完成后 0.5 天并行交付。M3 启动台账 + A+B 裁定已随组落档（`4d48439` → `723288f`，零分叉）：B 草案 v0.3 定稿（`docs/m3-b-sse-incremental-events-draft.md`，§九 勘误登记 3 处，其中 §四.4 行号按 @Hermes 终审裁定回滚）+ `docs/phase4-plan.md` v0.2.5 §4·B 增补（§4·B.6 勘误行 ① 当日计数口径三处 COUNT 行号终审裁定 = `:205/:248/:273`（SQL 字符串字面量行口径；@Hermes 2026-09-16 `HEAD bcd202c` 工作树直读裁定——v0.3 一度按 @Codex 核认报告误引参数行 `:206/:249/:274`，已回滚作废仅作版本对照保留，「台账漂移 +1 行」结论作废）；`since` 定义 `quota.py:17` 不变；config.py 完整路径 `backend/app/core/config.py:12-15/:18-27/:75-76`）+ @Codex 实码核认 + 工程排期/风险清单（`docs/m3-a-b-engineering-schedule-risk.md` 新增，M3 收口同 commit 落库）；@Hermes 第 4 步终审收口裁定 = 第 2/3 步交付物全部通过、零打回项。基座 `098cde9` 不动、HEAD `bcd202c`（M2 全链，零分叉）；@JARVIS 2026-09-16 裁定 M3 范围 **A+B**（主链路前置核认 + B SSE 增量事件协议草案）+ @Hermes 启动令 4 步序列。M2 完整闭环 ✅（origin/master `b369e1f`，零分叉）：M2 PR 1 代码批次（`a02e8ce`+`6ba0950`，含工程核认 2 条提醒落码 + 单测 12/12 + E2E 402 改判用例）+ M2 PR 2 前端批次（`ad0cc35` 实施 + `362420f` 终审复核 5 项勘误/修复：① `estimateCreditsRequired` 补 `complexity` 参数（multimodal=6）对齐后端；② 预扣门控补 `free_allowed` 门控（free 未耗尽时不预扣，修复 200 用例误判 402 回归 bug）；③ `quota/status` `credits.required` 对齐 §3.5.1 LIGHT=1；④ 402 判定式收敛为 `not allowed and 0<balance<required`；⑤ E2E setup-balance 路由勘误（`/api/quota/test/setup-balance`）+ 新增 `pytest.ini` 收口 `test_api.py` 4 项 async 收集失败）+ cleanup 批次 #1 核销（`b369e1f`）全部落库。回归三项全绿：后端 48/48 + Vitest 185/185（含 creditPanel.test.ts 12 项）+ E2E 56/56（402 用例命中确认）。Hermes 终审裁定：PR 1 有条件放行（口径 B）→ 复核后 PR 2 无条件放行 → M2 完整闭环收口（seq 18）。cleanup #2（E2E 402 命中重跑）非阻塞挂 Codex 台账
**M4 文档补录（@Claude，@Hermes v0.7 方案裁定照单执行，2026-09-16，零代码变更）**：`docs/m3-a-b-engineering-schedule-risk.md` 升 v0.7（generation.py 行号基准补录取回 v0.6 `:751` 系列 + quota.py 归属维持 v0.5 终审口径，v0.6 归属更正部分经 `2a7a5bd` 实码复核作废丢弃；`docs/phase4-plan.md` 升 v0.2.5-generation（§4·B.6 新增勘误行 ④ generation.py 行号基准补录全表 + ①③ 补 v0.6 作废归注，M4 B 线执行基准行号 = `:751/:756/:757/:759/:761-772/:776-782/:792-806/:822` + 总线侧 `:45/:56-64/:67-75/:79-131/:132-191/:136-138`，`quota.py` 行号维持终审基准 `:205/:248/:273` 不变）；B 草案 `docs/m3-b-sse-incremental-events-draft.md` v0.3 不动（§九 回滚注已闭环，M4 落码时随 commit 再核规则已写入）；STATUS.md BOM 口径行登记随组并入。远端偏差处置：`a6ae7fa`（v0.6 误归 commit）已随 revert 本地落库并 push origin（方式 B，审计链完整）；`docs/m4-b2-collabstream-prewrite.diff` 等 M4 B 线预写材料不属本 commit 范围，维持 `2a7a5bd` 归档
**最新 Commit**: M3 收口 commit 已拟（本文件随 @Hermes 第 4 步收口同 commit 落库：`STATUS.md` 终审收口裁定 + @Codex 工程排期/风险清单 `docs/m3-a-b-engineering-schedule-risk.md` 新增 + 当日计数口径行号终审裁定 = `:205/:248/:273`（SQL 字符串字面量行口径，@Hermes 2026-09-16 工作树直读裁定；v0.3 一度误引参数行 `:206/:249/:274` 已回滚作废，「台账漂移 +1」结论作废）+ BOM 补回登记；commit message = `docs(m3): B SSE 增量事件协议草案 §4·B 定稿 + 变更记录 v0.2.4 + 实码核认 + 风险清单落库`，由 @Hermes 执行）/ `723288f`（docs(m3): B 草案升 v0.3 + phase4-plan §4·B.6 勘误行 ① 行号勘误同步（随组 M3 收口同 commit））/ `4d48439`（docs(m3): B 草案 v0.2 定稿——路径全量化 + generation.py/quota.py 行号勘误 + §八勘误登记，零代码变更）/ `bcd202c`（chore(status) 补回 STATUS.md BOM / 台账对齐 `b369e1f` M2 收口）/ `b369e1f`（cleanup #1 落码——estimate_required 签名三方偏差核销）/ `cec8972`（STATUS.md BOM 修复 + cleanup 批次台账登记）/ `93b013b`（M2 PR 2 终审复核 5 项台账行落档）/ `362420f`（M2 PR 2 终审复核 5 项勘误/修复落码）/ `ad0cc35`（M2 PR 2 实施）/ `6ba0950`（M2 PR 1 收口）/ `a02e8ce`（M2 PR 1 实施）/ `752f974`（M2 §3.5 完整版正式落档）/ `fd5437a`（M2 启动台账行）/ `f668144`（§3.5 初稿，名实不符由 `752f974` 补正）/ `ba61f63`（M1 收口行落档）
...
- **启动令第 3 步（@Claude）已交付**：
  ...
  - **M3 收尾批次（`4d48439` → `723288f`，随组 M3 收口同 commit 零分叉）**：B 草案 v0.1（`4d48439` 首次落库）→ v0.2 定稿（`4d6e844`，路径全量化 + §八 6 处行号勘误逐条落实）→ 实码核认复核升 v0.3（`723288f`：§四.4 当日计数口径三处 COUNT 行一度改 `:206/:249/:274`，@Hermes M3 终审裁定回滚为 `:205/:248/:273`（SQL 字符串字面量行口径，`HEAD bcd202c` 工作树直读；v0.3 误引参数行作废仅作版本对照，「台账漂移 +1 行」结论作废），§四.1「调用点」措辞勘误归位（函数定义 `quota.py:23` / 调用点 `generation.py:685`），§四.2 单测锚点补 `test_quota_m2.py:99-100` `full_control` 同路径双行验证点）；`docs/phase4-plan.md` §4·B.6 勘误行 ① 同步终审裁定值 `:205/:248/:273`；BOM 头行登记（`STATUS.md` `EF BB BF` 已补回 `bcd202c`，`docs/phase4-plan.md` 随 M3 收口 commit 由 @Hermes 终审核一次）
  ...
  - **实码勘误行（M4 落码时随 commit 再核）**：当日计数口径三处行号终审裁定（@Hermes 2026-09-16，HEAD `bcd202c` 工作树直读）= `:205`（`get_quota_status_sync`）/ `:248`（`check_credits`）/ `:273`（`check_quota`），SQL 字符串字面量行口径（`since = _today_start_utc()` 定义 = `quota.py:17` 不变）；v0.3 一度按 @Codex 核认报告误引参数行 `:206/:249/:274`，已回滚作废仅作版本对照保留（「台账漂移 +1 行」结论作废；@Codex 误因溯源归因 = 实码核认报告「COUNT(*) 行」引用了 SQL 参数行，非 COUNT 字面量行，工程侧报告统一采用「SQL 字符串字面量行」口径标注）；M4 落码 PR 合入后再随 commit 复核一次
...
- **M3 第 4 步终审收口裁定（@Hermes，2026-09-16）**：按 A+B 全量口径收口，第 2/3 步交付物全部通过、零打回项。勘误生效规则（M3/M4 文档引用统一基准，@Codex 排期/风险清单直接引用无需再核）：① 当日计数口径三处终审裁定 = `quota.py:205`（`get_quota_status_sync`）/ `:248`（`check_credits`）/ `:273`（`check_quota`），SQL 字符串字面量行口径（`HEAD bcd202c` 工作树直读实测），`since = _today_start_utc()` 定义 = `quota.py:17` 不变（v0.3 一度按 @Codex 报告误引参数行 `:206/:249/:274`，@Hermes 复审裁定回滚作废，仅作版本对照）；② `config.py` 完整路径 = `backend/app/core/config.py:12` `_PROJECT_ROOT` / `:15` `_DB_PATH` / `:18-27` `_anchor_db_url` / `:75-76` 收尾锚定；③ BOM 补回登记：`STATUS.md` 头行 BOM（`EF BB BF`）已于 `bcd202c` 补回；`docs/phase4-plan.md` 当前无 BOM，随本次 M3 收口 commit 补回；④ `docs/m3-a-b-engineering-schedule-risk.md`（@Codex 定稿）文件编码为 GBK 存盘，已按 UTF-8 重编码（零 BOM），内容零改动，commit 后以 git diff 验证内容等价
- **M3 剩余交付物（@Codex / @Hermes）**：✅ 已全部闭环——第 2 步 = @Codex 工程排期（A 4 项 + B 3 项合并 7 项，含 owner/轮次/前置依赖/收口时间）+ 风险清单（DDL 勘误 R-DDL-1/R-DDL-2 + 429/402 路径分离标注 + R1-R5 + 三条收口硬约束 ①-④ 收录，定稿落 `docs/m3-a-b-engineering-schedule-risk.md`，M3 收口同 commit 落库）；第 4 步 = @Hermes 第 4 步终审收口裁定（2026-09-16）：B 草案 v0.3 + phase4-plan §4·B v0.2.5 + Codex 排期/风险清单 + STATUS 台账同步全部通过、零打回项，M3 收口 commit 由 @Hermes 统一落库；**唯一开放输入 = Gamma 竞对核实范围（竞对名单 + 数据维度）待 @JARVIS 确认**（@Codex 并行 1 天不阻塞主链）；Gamma 产出落档 `docs/competitive-analysis-v3.md`（M4 窗口前 1 天，含协作/presence 新增对标维度）
- **BOM 核验差异登记（@Hermes BOM 核验口径终审裁定，2026-09-16，本台账 commit 随组登记；M4 落码 PR 前统一核验）**：① `STATUS.md` 头行 BOM（`EF BB BF`）已于 `bcd202c` 补回，当前状态 = **有 BOM**；② `docs/phase4-plan.md` = **无 BOM（历史既成状态，`4d48439` 落库形态）——不追溯补 BOM，维持现状**；③ `docs/m3-b-sse-incremental-events-draft.md` = **无 BOM（同上，维持现状，不追溯补 BOM）**；④ `docs/m3-a-b-engineering-schedule-risk.md` = 无 BOM（UTF-8 重编码形态，随 M3 收口 commit 首次落库）；**核验口径**：仅 `STATUS.md` 保持 BOM 头行既有规范，三份 docs 不追溯，M4 落码前统一核验（④ `docs/m3-a-b-engineering-schedule-risk.md` 随 v0.7 补录修订核验：UTF-8 无 BOM 维持，内容零编码变化）
