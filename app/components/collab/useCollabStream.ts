...
export type CollabEvent =
  ...
  | { type: "generation_progress"; data: { stage: string; detail: string; pages?: number } }
...
export function useCollabStream(sessionId: string | null, enabled = true) {
  ...
  useEffect(() => {
    ...
    const onEvent = (type: CollabEvent["type"]) => (e: MessageEvent) => {
      const event = { type, data: JSON.parse(e.data) } as CollabEvent;
      setState((prev) => {
        const next = { ...prev, lastEvent: event, connected: true };
        if (type === "generation_progress") {
          next.progressStages = [...prev.progressStages, event.data.stage];
        }
        if (type === "collab_status") {
          next.status = event.data.status;
          next.statusMessage = event.data.message ?? null;
        }
        return next;
      });
    };
